...
dataHotspots %>%
  summarise( `Date range` = paste(min(firstDate), max(firstDate), sep = " - "),
             `Duration` = max(firstDate) - min(firstDate),
             `Total number of hotspots` = length(levels(hotspot))) %>%
  t() %>%
  kable(caption = "Table 1: Summary statistics on the content of the hotspot dataset.")
```

The first statistics we can calculate is the number of hotspot per owner. An owner is an Helium wallet to which several hotspots can be linked. 
Some owner really own **a lot** of hotspots. But that's just the top of the iceberg, let's see how we can better summarise this.
...
If we plot an histogram with all the counts, it will be difficult to visualize anything. Indeed, the distribution is hyper skewed. Most owner have only 1-2 hotspots but some have more than 100, up to 2000 (who are they?). To visualize the distribution, we restrict here to owners with *less* than 100 hotspots. We see that most owners have a limited number of hotspot (remember that the y axis is logarithmic).
...
**Let's summarise this in a better way (X% of... account for X% of)**

500k hotspots is a lot but there has not always been so many of them. Below, we visualize the growth of the network in terms of number of hotspots added to the network.
...
map +
  ...
  labs(title = "Localisation of hotspots in the world",
...
dataHotspots %>%
  ...
  arrange(desc(count)) %>%
  kable(caption = "Table 2: Distribution of hotspots per continent.")
```
...
**Understand why the duration of this dataset is smaller than the one above**
...
dataPacketsWithLocation %>%
  ...
  t() %>%
  kable(caption = "Table 1: Summary statistics on the content of the transaction dataset.")
```

To determine how often the hotspots are used by the devices, we chose here to analyse the number of transactions. Each data transfer between a hotspot and a device corresponds to one transaction on the blockchain. 

To summarise the evolution of the number of transaction, we use the cumulative sum function on the number of transaction per date. We further stratify by continent. The plot is very similar to the plot above for the number of hotspot: a slow linear phase followed by an exponential and then a fast linear phases (but what is this glitch at the end?). Surprisingly, we see that despite having about 15% of the hotspots, Asia don't seem to be so active in terms of data transfer in contrast to North America and Europe.

```{r, fig.cap = 'Figure X: Growth of the number of transactions between hotspots and devices, stratified by continent.'}
# count the number of transaction per continent and date and calculate a cumulative sum
nTransactionsPerDatePerContinent <- dataPacketsWithLocation %>%
...
```{r}
summaryTransactionPerHotspot <- dataPacketsWithLocation[, .(count = .N), 
...
propWith0Transactions <- length(which(table(dataPacketsWithLocation$hotspot) == 0)) / 
  length(levels(dataPacketsWithLocation$hotspot)) * 100
```
...
As we did above, we can plot the world map, bin the data using the *makeHexData* function and overlay with the number of data transactions. This time, let's create a longitudinal animation using the *gganimate* package. Altough a direct comparison with Figure X is difficult since we have here an additional dimension (the colour refers to the number of transactions), this figure gives a similar message. We see that transactions are mainly done in North America before mid 2020. We then start seeing a strong increase in Europe and Asia. Nothing is visible in South America and Africa. 

```{r, fig.cap = 'Figure X: Evolution of the number of transactions across the globe.', fig.width = 10, fig.height = 6}
## bin the hotspot into hexagons
# find the bounds for the complete data
...
To add a bit of perspective, we can also turn the plot in 3D with the awesome *rayshader* package. Let's focus on two countries: (1) US as it has the biggest number of hotspots and transactions and (2) Belgium, which is my home country. We have to rebin the data into hexagons since we will generate a static plot and not an animation. It is possible to animate this plot but it's gonna take a lot of time and computing power (see [this](https://ghanadatastuff.com/post/3d_population_density/)).

This is US. We see that transactions are well distributed across the country although the peaks are located around big cities (New york, Los Angeles, San Francisco, Miami).

```{r, fig.cap = 'Distribution of the transactions in US. The view can be manipulated with the mouse.'}
...
This is Belgium. Here the pattern is different as we see that transactions are not homogeneously distributed in the country. Most transactions happen in the upper part of the country, the lower part being not densely populated.
...
We saw how to summarise Helium usage, how to work with coordinates and convert them into continent/country. How to plot this is animated and 3D plots.
