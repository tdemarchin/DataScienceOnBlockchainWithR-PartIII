...
map +
    geom_point(aes(x = lng, y = lat, col = count),
               size = 0.5,
               alpha = 0.5,
             data = nHotspots) + 
...
Let's summarise this in a nice table. For this, we will leverage the *maps* package.
TODO: some countries cannot be found. Convert to continent.

```{r}
dataHotspot <- dataHotspot %>%
  mutate(country = map.where(database="world", .$lng, .$lat),
         country = replace_na(country, "Undefined"))
